...
class CrackDetect:
    ...
    def outline_crack(self, image_path, save_subset=False):
        ...
        folder_name = os.path.dirname(image_path) + "_results" 
        if not os.path.isdir(folder_name):
            os.mkdir(folder_name)
        cv2.imwrite(folder_name + "/" + file_name, polygons)
...
begin region_of_interest
    ...
    #creates outlines for all images in a folder (does not show preview or save subset file)
    def outline_crack_batch(self, folder_path):
        files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if (os.path.isfile(os.path.join(folder_path, f)) and (os.path.splitext(f)[1] == ".tif" or os.path.splitext(f)[1] == ".tiff"))]
        for f in files:
            if(self.has_crack(f)):
...
def main():
    ...
    detector.outline_crack_batch(sys.argv[1])
